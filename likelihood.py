...
def get_likelihood_fn(sde, inverse_scaler, hutchinson_type='Rademacher',
  ...
  def drift_fn(model, x, t):
    ...
    return sde.reverse_sde(score_fn, x, t)[0]
  ...
  def likelihood_fn(model, data):
    ...
    with torch.no_grad():
      ...
      print(zp.shape)
      logp_traj = solution.y[-1,:]
      print(logp_traj.shape)
      print(logp_traj[-1])
      # logp_traj = solution.y[]
      ...
      print(delta_logp)
      logp_traj = logp_traj + (torch.ones_like(prior_logp) * prior_logp).cpu().numpy()
